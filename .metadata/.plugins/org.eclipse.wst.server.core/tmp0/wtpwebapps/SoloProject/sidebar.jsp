...
<body>
	...
	<div class="d-flex flex-column flex-shrink-0 p-3 bg-light" style="width: 280px;">
		...
		<% for (int i=0; i<8; i++) {
			if (sideIdx == i) {
			%>
			<li><a href="main.jsp?sideIdx=<%=i %>" class="<%=active %>" style="<%=active_background%>"><%=str[i] %></a></li>
			<%	
				if (i==0) { %><hr><% }
			} 
			else {
			%>
			<li><a href="main.jsp?sideIdx=<%=i %>" class="<%=passive %>"><%=str[i] %></a></li>
			<%
				if (i==0) { %><hr><% }
			}
		}
		%>
			<hr>
			<li><a href="#" class="<%=passive %>">문의하기</a></li>
		</ul>
		<hr>
	</div>
